        ...
        if let indexPath = lastTopVisibleCellIndexPath{
            ...
            if addedNewSection{
                if indexPath.section == 0 && indexPath.row == 0{
                    ...
                    let newSection = 0
                    self.scrollToRow(at: IndexPath(row: quant - 1, section: newSection), at: UITableViewScrollPosition.top, animated: false)
                }
                else{
                    let newSection = indexPath.section + addedSectionsValue
                    self.scrollToRow(at: IndexPath(row: indexPath.row, section: newSection), at: UITableViewScrollPosition.top, animated: false)
                }
                ...
                if indexPath.section == 0{
                    ...
                    self.scrollToRow(at: IndexPath(row: newRow, section: indexPath.section), at: UITableViewScrollPosition.top, animated: false)
                }
                else{
        ...
        if frame.height > 0{
            ...
            for i in 0..<self.numberOfSections{
                ...
                if let sectionHeader = self.headerView(forSection: i){
                    sectionHeader.layer.delegate = self.layer.delegate
                ...
                if let sectionFooter = self.footerView(forSection: i){
                    sectionFooter.layer.delegate = self.layer.delegate
                    UIView.animateKeyframes(withDuration: 0.25, delay: 0, options: UIViewKeyframeAnimationOptions(), animations: {
            ...
            if let indexPaths = self.indexPathsForVisibleRows{
                for indexPath in indexPaths{
                    if let cell = self.cellForRow(at: indexPath){
                        cell.layer.delegate = self.layer.delegate
                        UIView.animateKeyframes(withDuration: 0.25, delay: 0, options: UIViewKeyframeAnimationOptions(), animations: {
